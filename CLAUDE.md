...
スマホのClaudeアプリでもデスクトップと同じ読み方ができるよう、返答は下記の形式で統一する。

- **結論を最初の1〜2行に書く**（最重要）。「何をしたか／どうなったか」を冒頭で言い切り、理由・経緯・詳細は必ずその後ろ。長い説明の末尾に結論を置かない。
- 作業報告は `やったこと` → `結果` → `次にどうするか` の見出し順を固定。該当なしの節は省略してよいが順序は変えない。
- 表は使わず箇条書きに展開する（スマホ幅で潰れるため）。箇条書きの入れ子は2階層まで。
- 1文は40〜50字程度で改行し、横スクロールを出さない。
- コードブロックは変更の要点となる数行のみ。差分の全文貼り付けはしない。まず日本語で「どのファイルの何を変えたか」を書く。
- ファイル参照は `js/parser.js:120` 形式で統一する。
- テスト結果は合否を明示する（`node tests/parser.test.js` の結果を隠さない）。
- **中学生が読んで分かる言葉で解説を添える**。専門用語（マーカー・パーサー・ゼロ幅文字・コミット等）を使ったら、その場でかっこ書きか次の行に一言で言い換える。例：「パーサー（＝文章を読み取って部品に分ける処理）」。
- 「なぜそうするのか」を1行添える。手順だけを並べて理由を省略しない。
- 比喩を使ってよい（例：ゼロ幅IDは「透明な名札」）。ただし比喩だけで終わらせず、実際の動作も併記する。
