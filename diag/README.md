...
### 8-6. 実機での3方式の突き合わせ結果（2026-09-22・Premiere 26.0.2・実案件1本）

基準＝Adobe「テキスト→グラフィック→書き出し→CSV」37件（UTF-8・CRLF・列＝開始/終了TC・テキスト・Layer ID。**トラック列なし**・文中改行はCR）。

| 方式 | 件数 | 完全一致 | 一致率 | 取りこぼし | 余分 |
| --- | --- | --- | --- | --- | --- |
| A：CEP 診断（ComponentParam.getValue） | 73 | 2 | **5.4%** | 35 | 71 |
| H：.prproj 直読み（改行をLFにそろえて比較） | 60 | 35 | **94.6%** | 2 | 25 |

- **A が Premiere 製テキストを1件も取れない理由（実測）**：「ソーステキスト」の `getValue()` が60部品すべてで**長さ1の文字**を返す。その文字は .prproj 側 FlatBuffer の先頭2バイトと60/60一致＝直後の 00 00 で文字列が切れている。レイヤー名も全部「テキスト」、クリップ名も全部「グラフィック」で代用にならない。→ **Premiere 26.0.2 では CEP から本文は読めない**（TelopStyle の実運用は旧版・旧形式の頃の話と見られる＝推測）。8-3 の食い違いは TelopSkin 側の記述が正しかった。
- **A の余分71件**は全部ゴミ（Lumetri「Blob」・Shape のバイナリ断片・上の1文字）。extract.js の除外判定が1文字ゴミを通す。
- **H の取りこぼし2件は AE 製 MOGRT（`AE.ADBE Capsule`）**。H は `AE.ADBE Text` しか読まないため。本文は .prproj 内の base64（UTF-16 の JSON・`textEditValue`）にあることを確認＝**H を広げれば読める見込み**。MOGRT は A（`getMGTComponent()` の JSON の `textEditValue`）なら2/2取れた。
- **H の余分25件**は全部「1クリップ内の2枚目の文字レイヤー」（話者名「(柔道整復師)」18・「(理学療法士)」6・「→」1）。Adobe CSV は各グラフィックの1枚目しか出していない（推測・画面に出ているかは未確認）。誤字チェックには2枚目も必要＝H のほうが基準より多く拾える。
- 一致した分は開始・終了ともフレーム単位で全件一致。トラックは A と H で35クリップ全部一致（H の track は0始まり）。
